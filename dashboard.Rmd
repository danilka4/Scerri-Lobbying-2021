...
```{r}
voiced17 <- data_creator_vv(null17)
labels_17 <- c("Introduced", levels(voiced17$x)[2:4], "Other Committee",  "Passed Floor 1", "Passed Committee 2", "Passed Floor 2", "Delivered to Governor", "Signed by Governor", "Law")
plot_ly(
  type = "sankey",
  arrangement = "snap",
  node = list(
    ...
    pad = 10),
  link = list(
    source = as.numeric(voiced17$x) - 1,
    target = as.numeric(voiced17$next_x),
    customdata = flow_names(voiced17),
    value = voiced17$n,
    color = ~as.factor(voiced17$color),
    hovertemplate = link_hover,
    line = list(color = "black", width = 0.5)
    )) %>%
  layout(title = "Voiced Votes for Education in 2017",
         xaxis = list(showgrid = F, zeroline = F),
         yaxis = list(showgrid = F, zeroline = F),
         showlegend = T)
```
